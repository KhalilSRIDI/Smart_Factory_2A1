    ...
    ui(new Ui::statis)
{
    ...
    regen->setName("Modele/Nombre De Maintenance");
    ...
    q.prepare("SELECT count(modele)  FROM MATERIEL WHERE nombre_maintenance >2 ");
    if(q.exec()){
    while (q.next()) {
    ...
    ui->widget->xAxis->setLabel("Modele");
    ...
    ui->widget->yAxis->setLabel("Nombre De Maintenance");
...
void statis::on_pushButton_clicked()
{
    hide();
